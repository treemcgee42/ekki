...
use rend3_routine::{common, culling, forward::RoutineAddToGraphArgs, pbr, skinning, skybox, tonemapping};
...
impl BaseRenderGraph {
    ...
    pub fn add_to_graph<'node>(
        ...
        pbr: &'node rend3_routine::pbr::PbrRoutine,
        skybox: Option<&'node rend3_routine::skybox::SkyboxRoutine>,
        tonemapping: &'node rend3_routine::tonemapping::TonemappingRoutine,
        ...
        let (state, depth_target) = BaseRenderGraphIntermediateState::new(graph, eval_output, resolution, samples);
...
impl BaseRenderGraphIntermediateState {
    ...
    pub fn create_frame_uniforms<'node>(
        ...
        rend3_routine::uniforms::add_to_graph(
            graph,
    ...
    pub fn skinning<'node>(&self, graph: &mut RenderGraph<'node>, base: &'node BaseRenderGraph) {
        rend3_routine::skinning::add_skinning_to_graph(graph, &base.gpu_skinner);
    }
    ...
    pub fn clear<'node>(&self, graph: &mut RenderGraph<'node>, clear_color: Vec4) {
        rend3_routine::clear::add_clear_to_graph(graph, self.color, self.resolve, self.depth, clear_color, 0.0);
    }
